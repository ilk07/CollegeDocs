CollegeDocs.utils.renderBoolean = function (value) {
    return value
        ? String.format('<span class="green">{0}</span>', _('yes'))
        : String.format('<span class="red">{0}</span>', _('no'));
};

CollegeDocs.utils.getMenu = function (actions, grid, selected) {
    const menu = [];
...
CollegeDocs.utils.renderActions = function (value, props, row) {
    const res = [];
    ...
    for (const i in row.data.actions) {
        ...
        item = String.format(
            '<li class="{0}"><button class="collegedocs-btn collegedocs-btn-default {1}" action="{2}" title="{3}"></button></li>',
    ...
    return String.format(
        '<ul class="collegedocs-row-actions">{0}</ul>',
        res.join('')
    );
};